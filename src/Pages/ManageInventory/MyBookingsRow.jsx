const MyBookingsRow = ({ booking,handleDelete}) => {
  const { _id,serviceName, img,email, serviceDate } = booking;
  return (
    <tr>
      <th>
        <label>
      ...
      <td className="font-semibold text-base">{email}</td>
      <td className="text-base">{serviceDate}</td>
      <th>
    
      
          <button className="btn border-[#FF3811] bg-[#FF3811] hover:bg-[#FF3811] hover:border-[#FF3811] text-white btn-sm ">Approve</button>
      </th>
    </tr>
  );
};
